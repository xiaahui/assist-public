...
class DownBranchMonitor implements IMonitorDownBranch {
	...
	def calculateProgress() {
		...
		if (Math.abs(currentProgress - bestProgress) >= 1) {
			...
			logger.info('''Search progress: «String::format("%3d", currentProgress)»% of all location variables and «String::format("%3d", colocProgress)»% of the colocation variables are instantiated''')
