...
class DeploymentAsSchedulingModel {
    ...
    static def AssistModel create(MappingResult mappingResult) {
        ...
        clonedModel.systemName = oldModel.systemName + " - Mapping Solution " + (mappingResult.index + 1)

        // Remove attributes from the mapping which are no longer used in scheduling
        for (task : clonedModel.allTasks) {
            val coreUtilProperties = task.properties.filter[it instanceof CoreUtilizationProperty]
            task.properties.removeAll(coreUtilProperties)
        }
