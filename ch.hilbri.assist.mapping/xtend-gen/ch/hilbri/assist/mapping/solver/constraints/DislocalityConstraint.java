...
public class DislocalityConstraint extends AbstractMappingConstraint {
  public DislocalityConstraint(final AssistModel model, final Store constraintStore, final SolverVariablesContainer solverVariables) {
    ...
    for (final DislocalityRelation r : _dislocalityRelations) {
      {
        ...
        for (final ArrayList<IntVar> list : varSetForAllDifferentConstraint) {
          Alldifferent _alldifferent = new Alldifferent(list);
          this.constraintStore.impose(_alldifferent);
        }
      }
    }
    return true;
