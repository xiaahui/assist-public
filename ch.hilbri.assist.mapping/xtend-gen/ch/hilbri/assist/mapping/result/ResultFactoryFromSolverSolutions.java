...
public class ResultFactoryFromSolverSolutions {
    ...
    for (final ch.hilbri.assist.datamodel.model.Thread thread : _allThreads) {
      {
        ...
        final Integer coreIndex = solution.getIntVal(locVar);
        EList<Core> _allCores = model.getAllCores();
        Core _get = _allCores.get((coreIndex).intValue());
        ...
        if (_equals) {
          ...
          Core _get_1 = _allCores_1.get((coreIndex).intValue());
    ...
    for (final ch.hilbri.assist.datamodel.result.mapping.Core resultCore : _allCores) {
      {
        ...
        final Integer absoluteUtilization = solution.getIntVal(_absoluteCoreUtilizationVariable);
        resultCore.setUtilization((absoluteUtilization).intValue());
      }
    }
    ...
    for (final ch.hilbri.assist.datamodel.result.mapping.Board resultBoard : _allBoards) {
      {
        ...
        final Integer absoluteRamUtilization = solution.getIntVal(_absoluteRamUtilizationVariable);
        resultBoard.setRamUtilization((absoluteRamUtilization).intValue());
      }
    }
    ...
    for (final ch.hilbri.assist.datamodel.result.mapping.Board resultBoard_1 : _allBoards_1) {
      {
        ...
        final Integer absoluteRomUtilization = solution.getIntVal(_absoluteRomUtilizationVariable);
        resultBoard_1.setRomUtilization((absoluteRomUtilization).intValue());
      }
    }
  }
  ...
  public static ArrayList<Result> createPartialResult(final AssistModel model, final SolverVariablesContainer solverVariables, final Map<IntVar, Integer> solverSolutions) {
    return null;
  }
}
