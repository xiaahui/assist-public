...
public enum VariableSelectorTypes {
	...
	DOM_OVER_WDEG("Domain over weighted degree", "", "DOWD", false, true), 
	MIN_DOMAIN_FIRST("Min domainsize first", "", "MNDF", false, true), 
	RANDOM("Random", "", "RAND", false, true),
	RATE_MONOTONIC("Rate monotonic", "", "RMNT", true, true);
	...
	VariableSelectorTypes(String name, String explanation, String cliArgumentName, boolean isDefault,
			boolean isValueSelectorRequired) {
	...
	public boolean isValueSelectorRequired() {
		return isValueSelectorRequired;
	}

	public AbstractStrategy<IntVar> getStrategy(SolverVariablesContainer solverVariables, AssistModel assistModel,
			long seed, ValueSelectorTypes valSelector) {

		IntVar[] vars = solverVariables.getAllVariables();

		switch (this) {
		case DOM_OVER_WDEG:
			return new DomOverWDeg(vars, seed, valSelector.getValueSector(solverVariables, assistModel, seed));
		case RANDOM:
			return Search.intVarSearch(new Random<>(seed),
					valSelector.getValueSector(solverVariables, assistModel, seed), vars);
		case MIN_DOMAIN_FIRST:
			return Search.intVarSearch(new FirstFail(vars[0].getModel()),
					valSelector.getValueSector(solverVariables, assistModel, seed), vars);
		case RATE_MONOTONIC:
			return Search.intVarSearch(new RateMonotonicVarSelector(assistModel, solverVariables, vars), 
					valSelector.getValueSector(solverVariables, assistModel, seed), vars);
		}
	...
	public static VariableSelectorTypes getVariableSelectorOrDefault(String cliArgumentName) {
		...
		case "RAND":
			return RANDOM;
		case "DOWD":
			return DOM_OVER_WDEG;
		case "MNDF":
			return MIN_DOMAIN_FIRST;
		default:
			return getDefault();
		}
	}
	...
	/*
	 * Sort the values, so that the default strategy is first and the remaining
	 * strategies are sorted by name
	 */
		...
		Collections.sort(list, new Comparator<VariableSelectorTypes>() {
			...
			public int compare(VariableSelectorTypes o1, VariableSelectorTypes o2) {
				if (o1.isDefault())
					...
					return o1.humanReadableName.compareTo(o2.humanReadableName);
			}
		});
