...
class ExportToText {
        ...
        try {
            if (fileName.fileExtension == "txt") {
                ...
                MessageDialog.openInformation(shell,
                    "Export successful", '''The result with the name '«result.name»' was successfully exported to file '«fileName»'.''')
            } else {
                MessageDialog.openError(shell,
                    "Export unsuccessful", '''The result with the name '«result.name»' could not be exported to '«fileName»', because the extension «fileName.fileExtension» is unknown.''')
            }
    ...
    private def String getFileExtension(String fileName) {
        if(fileName.lastIndexOf('.') > 0) fileName.substring(fileName.lastIndexOf('.') + 1) else ""
    }
}
